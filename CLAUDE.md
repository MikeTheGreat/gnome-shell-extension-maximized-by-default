...
The UUID is `gnome-shell-extension-maximized-by-default@stiggimy.github.com`. When adding support for a new GNOME Shell version, update `shell-version` in `metadata.json` and verify the `maximize()` API hasn't changed again.
